...
#define _DEFINE_EM_JS_INLINE(ret, c_name, js_name, params, code)                          \
  ret c_name params EM_IMPORT(js_name);                                            \
  EMSCRIPTEN_KEEPALIVE                                                             \
  __attribute__((section("em_js"), aligned(1))) char __em_js__##js_name[] = \
    #params "<::>" code;

#define DEFINE_EM_JS_INLINE(ret, name, params, ...) _DEFINE_EM_JS_INLINE(ret, name, name, params, #__VA_ARGS__)

DEFINE_EM_JS_INLINE(void, emscripten_upload, (char const *accept_types, upload_handler callback, void *callback_data), {
...
DEFINE_EM_JS_INLINE(void, emscripten_download, (char const *filename, char const *mime_type, void const *buffer, size_t buffer_size), {
...
EMSCRIPTEN_KEEPALIVE int upload_file_return(char const *filename, char const *mime_type, char *buffer, size_t buffer_size, upload_handler callback, void *callback_data) {
  ...
  if (!buffer || buffer_size == 0) {
    callback(filename, mime_type, NULL, 0, callback_data);
    return 1;
  }
  /// Ok
